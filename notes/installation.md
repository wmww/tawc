...
- offer the same operations to adb / integration tests via the
  `scripts/tawc-chroot-run.sh` host script (no broadcast surface)
...
- **Most config changes aren't critical for keeping the chroot
  ...
  without them.
- **Anything truly critical to keeping a chroot working is in
  `enter.sh`,** which is regenerated by `ChrootMounter.enterScript`
  on every chroot entry (`ChrootRunner.run`, `Installer.install`,
  and the host-side `scripts/tawc-chroot-run.sh` all invoke it). Mount
  layout, env vars, and the `wayland-0` symlink therefore *do*
  pick up app updates without a reinstall.
...
`scripts/tawc-chroot-run.sh` is the host-driven counterpart to in-app
