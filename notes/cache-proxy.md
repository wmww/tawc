...
Verification fetches (PGP `.sig`, ALARM `.md5` sidecars, GitHub
Releases REST API, Void's `sha256sum.txt`) **do** route through the
proxy in dev builds. We used to keep them off-proxy to preserve the
cross-mirror "two independent operators" integrity argument, but in
practice that produced a permanent verification mismatch the moment
upstream rotated an artifact (proxy serves stale tarball, fresh
digest fetched direct → md5 mismatch with no in-band recovery). Dev
trades the cross-check for cache coherence; release builds never
construct a `MirrorProxy` so the upstream-direct integrity story is
intact for users.
