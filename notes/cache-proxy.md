...
sweep it manually: `rm -rf build/cache-proxy/cache/`. Selective evict
by md5 path under `build/cache-proxy/cache/`. CLAUDE.md instructs
agents not to wipe it without explicit user request.
...
- The mirror config baked into the rootfs hard-codes the
  ...
  Re-run `bash scripts/cache-proxy.sh` (and let it apply
