...
```bash
bash scripts/tawc-rootfs-run.sh                    # interactive shell
bash scripts/tawc-rootfs-run.sh '<command>'        # run a command and exit
```

`tawc-rootfs-run` shells into `<installation-dir>/enter.sh` over `adb shell
su` (the rootfs is in the app's private data dir, so root is required to
read it). All bind mounts, su escalation, and profile setup are handled
inside `enter.sh`, which is auto-generated by `ChrootMounter` — the mount
logic lives in Kotlin only. Generic tawc Wayland env vars are set by
`/etc/profile.d/01-tawc.sh` (also rewritten on every chroot entry).

### Shell quoting through adb/su/chroot

Commands pass through multiple shell layers: host bash → adb → mksh → su → chroot bash.
`tawc-rootfs-run` base64-encodes the chroot command into `enter.sh`'s `$1`,
so any bytes in the command (quotes, newlines, …) survive the trip without
re-escaping. If you ever bypass the script (raw `su -c '…'` over adb), you
need to handle the layered quoting yourself:
...
- `install/` — Kotlin implementation of the chroot install / run /
  ...
  `scripts/tawc-rootfs-run.sh`, which shells into the same auto-generated
