...
- `tests/integration/src/chroot_process.rs:106` — `test -d /proc/<pid>` for liveness checks
- `tests/integration/src/chroot_process.rs:141` — reads `/proc/<pid>/<file>`
- `tests/integration/src/chroot_process.rs:153` — reads `/proc/<pid>/stat`
- `tests/integration/src/chroot_process.rs:169` — `rm -f` of the pidfile on the device
- `tests/integration/src/chroot_process.rs:209` — copies `tawc-pidfile-exec` into the rootfs and chmods it
...
- On an unrooted device (or emulator with Magisk disabled / `su` missing), `bash scripts/run-integration-tests.sh` against a proot install completes without prompting for superuser and without errors.
