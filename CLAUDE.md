...
- **Integration tests:** `bash scripts/run-integration-tests.sh [--graphics libhybris|gfxstream] [filter]` (builds everything, deploys, runs all tests; the optional positional arg is a libtest substring filter, e.g. `apps::` or `apps::test_firefox`. `--graphics` flips the in-app graphics-driver pref before the suite starts (default `libhybris`) and exposes it to tests via `TAWC_GRAPHICS_BACKEND` so libhybris-only assertions can `SKIP:` cleanly. The chroot-side gfxstream bits ride in the APK and are laid into each rootfs by `BridgeInstallProvider` at install time, and the kumquat server runs in the compositor process — no extra staging or daemon to manage.)
