...
- [android.md](notes/android.md) -- socket sharing, SELinux, rootfs setup
...
- Use existing scripts (eg `scripts/rootfs-run.sh`) instead of one-shotting commands
...
- **Build (gfxstream-bridge):** two scripts — `bash scripts/build-mesa-gfxstream.sh` (chroot-side `libvulkan_gfxstream.so` + ICD JSON, aarch64 glibc) and `bash scripts/build-gfxstream-backend.sh` (Android-side `libgfxstream_backend.so` + `libc++_shared.so`, NDK aarch64; staged into `app/src/main/jniLibs/arm64-v8a/`). The kumquat server itself is a Rust dep of the compositor (`compositor/Cargo.toml`'s `kumquat_virtio` entry, target-gated to aarch64), built as part of the cargo cross-compile and linked into `libcompositor.so`; it runs on a thread of the compositor process. Patches in `deps/gfxstream-patches/`, `deps/mesa-patches/`, `deps/rutabaga-patches/`. Gradle's `buildGfxstreamBackend` task wires the host renderer into `assembleDebug`; `buildMesaGfxstream` + `packMesaGfxstream` ship the chroot-side .so + ICD JSON as APK assets, and `BridgeInstallProvider` lays them into each rootfs at `/usr/lib/gfxstream/libvulkan_gfxstream.so` + `/usr/lib/gfxstream/gfxstream_vk_icd.aarch64.json` at install time (alongside libhybris) — no separate staging step. Backend selection lives in the in-app Settings screen (`GraphicsBackend` enum in `Settings.kt`, persisted in `SharedPreferences`); `RootfsEnv` reads it on every rootfs spawn. Tests pick a backend per-spawn via the broker `GRAPHICS` header on RUNINSIDE (`tawc-exec --in-rootfs <id> --graphics libhybris|gfxstream|cpu …` or `RootfsProcess::spawn_with` from the Rust harness); the persisted Settings pick is left alone.
...
- **Graphics backends:** all four (`libhybris`, `libhybris-zink`, `gfxstream`, `cpu`) ship by default. Build-time override: `-PtawcGraphics=libhybris,gfxstream,cpu` to drop the Mesa-Zink cross-build (~3-5 min off `assembleDebug` and ~22 MB off the APK). Wired through `EnabledGraphicsBackends` — Settings UI hides disabled entries, `MesaZinkInstallProvider` short-circuits to empty, persisted prefs that resolve to a disabled backend fall back to `GraphicsBackend.DEFAULT`. See `notes/libhybris-zink.md` "Build-time flag".
...
- **Rootfs (interactive):** `bash scripts/rootfs-run.sh`
...
- **Integration tests:** `bash scripts/run-integration-tests.sh [filter]` (builds everything, deploys, runs all tests; the optional positional arg is a libtest substring filter, e.g. `apps::` or `apps::test_firefox`). A single run exercises every graphics backend: the `hybris::` / `gfxstream::` / `cpu_graphics::` modules each pin their backend per spawn via the broker `GRAPHICS` header, and `apps::` / `input::` use CPU, `xwayland::` uses libhybris. The chroot-side gfxstream bits ride in the APK and are laid into each rootfs by `BridgeInstallProvider` at install time, and the kumquat server runs in the compositor process — no extra staging or daemon to manage.
